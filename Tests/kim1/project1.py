...
def test_Collage():
    doneCollage = makePicture("kim1.jpg")
    builtPic = createCollage()
    redDif = 0
    blueDif = 0 
    greenDif = 0
    largestDif = 0
    for y in range(builtPic.getHeight()):
      for x in range(builtPic.getWidth()):
        builtPix = builtPic.getPixel(x,y)
        donePix = doneCollage.getPixel(x,y)
        curRed = abs(builtPix.getRed()-donePix.getRed())
        curBlue = abs(builtPix.getBlue()-donePix.getBlue())
        curGreen = abs(builtPix.getGreen()-donePix.getGreen())
        redDif = redDif + curRed
        blueDif = blueDif + curBlue
        greenDif = greenDif + curGreen
        if(curRed > largestDif):
          largestDif = curRed
          bigX = x
          bigY = y
        if(curBlue > largestDif):
          largestDif = curBlue
          bigX = x
          bigY = y
        if(curGreen > largestDif):
          largestDif = curGreen
          bigX = x
          bigY = y
        
    print(redDif/(builtPic.getHeight()*builtPic.getWidth()))
    print(blueDif/(builtPic.getHeight()*builtPic.getWidth()))
    print(greenDif/(builtPic.getHeight()*builtPic.getWidth()))
    print(redDif)
    print(blueDif)
    print(greenDif)
    print(largestDif)
